...
namespace test {
    class TestInstructionFma : public TestInstruction {
    public:

        TestInstructionFma() {
            name = "TestInstructionMul";

            description = "A unit test for the MUL instruction.";
        }
    protected:
...
int main( int argc, char** argv )
{
	...
	test::TestInstructionFma test;
	parser.description( test.testDescription() );

	parser.parse("-c", test.configPath, "../instruction/mul/config_f16.test", "Test configuration path.");
	parser.parse( "-i", test.input, "../instruction/mul/test_mul.ptx",
		"Test PTX path.");
