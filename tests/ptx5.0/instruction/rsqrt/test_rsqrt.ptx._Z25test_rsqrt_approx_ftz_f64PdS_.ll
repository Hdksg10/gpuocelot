...
BB_1_2:
	...
	%rt8 = bitcast double %r4 to i64;
	%rt9 = lshr i64 %rt8, 32;
	%rt10 = trunc i64 %rt9 to i32;
	%rt11 = bitcast i32 %rt10 to float;
	%rt13 = fcmp olt float %rt11, 0x0;
	%rt14 = fsub float 0x0, %rt11;
	%rt15 = select i1 %rt13, float %rt14, float %rt11;
	%rt16 = fcmp olt float %rt15, 0x3810000000000000;
	%rt12 = select i1 %rt16, float 0x0, float %rt11;
	%rt7 = call float @llvm.sqrt.f32(float %rt12);
	%rt17 = fdiv float 0x3ff0000000000000, %rt7;
	%rt18 = bitcast float %rt17 to i32;
	%rt19 = zext i32 %rt18 to i64;
	%rt20 = shl i64 %rt19, 32;
	%rt21 = bitcast i64 %rt20 to double;
	%rt22 = fcmp olt double %rt21, 0x0;
	%rt23 = fsub double 0x0, %rt21;
	%rt24 = select i1 %rt22, double %rt23, double %rt21;
	%rt25 = fcmp olt double %rt24, 0x10000000000000;
	%r5 = select i1 %rt25, double 0x0, double %rt21;
