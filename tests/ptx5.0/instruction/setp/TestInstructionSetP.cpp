...
namespace test {
    class TestInstructionSetP : public TestInstruction {
    public:

	TestInstructionSetP() {
            name = "TestInstructionSetP";

            description = "A unit test for the SET instruction.";
        }
    protected:
...
int main( int argc, char** argv )
{
	...
	test::TestInstructionSetP test;
	parser.description( test.testDescription() );

	parser.parse("-c", test.configPath, "../instruction/setp/config_f16.test", "Test configuration path.");
	parser.parse( "-i", test.input, "../instruction/setp/test_setp.ptx",
		"Test PTX path.");
